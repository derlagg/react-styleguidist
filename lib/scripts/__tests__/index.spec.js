...
const getDefaultWebpackConfig = () => (0, _index.default)().makeWebpackConfig();
...
it('should return API methods', () => {
  const api = (0, _index.default)(require('../../../test/data/styleguide.config.js'));
...
describe('makeWebpackConfig', () => {
  ...
  it('should merge webpackConfig config option', () => {
    ...
    expect((0, _last.default)(result.resolve.extensions)).toEqual('.scss');
  });
  ...
  it('should apply updateWebpackConfig config option', () => {
    ...
    expect(result).toBeTruthy();
    expect(result.resolve.extensions.length).toEqual(defaultWebpackConfig.resolve.extensions.length + 1);
    expect((0, _last.default)(result.resolve.extensions)).toEqual('production');
  });
...
describe('build', () => {
  it('should pass style guide config and stats to callback', () => {
    ...
    const api = (0, _index.default)(config);
    api.build(callback);
