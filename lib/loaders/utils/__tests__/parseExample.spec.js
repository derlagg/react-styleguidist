...
it('should parse modifiers as JSON', () => {
  const actual = (0, _parseExample.default)(content, 'js', '{ "showcode": true }');
...
it('should parse modifiers as a space-separated string', () => {
  const actual = (0, _parseExample.default)(content, 'jsx', 'showcode static');
  expect(actual).toEqual({
    lang: 'jsx',
    settings: {
      showcode: true,
...
it('should return settings as an empty object', () => {
  const actual = (0, _parseExample.default)(content, 'js');
  expect(actual).toEqual({
    lang: 'js',
    settings: {},
    content
...
it('should accept language as null', () => {
  const actual = (0, _parseExample.default)(content, null);
  expect(actual).toEqual({
    lang: null,
    settings: {},
    content
...
it('should apply an update function', () => {
  const actual = (0, _parseExample.default)(content, 'js', 'coffee', a => Object.assign({}, a, {
    lang: 'pizza'
  }));
  expect(actual).toEqual({
    lang: 'pizza',
...
it('should return an error when JSON is invalid', () => {
  const actual = (0, _parseExample.default)(content, 'js', '{ nope }');
  expect(actual).toEqual({
